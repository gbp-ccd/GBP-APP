...
useEffect(() => {
  ...
  useEffect(() => {
  if (readyToSubmit && reflectionStep === 1) {
    setReflectionStep(2);
  }
}, [readyToSubmit, reflectionStep]);

  const handleAssign = (proposalId, bucket) => {
  setAssignments(prev => {
    const newAssignments = { ...prev, [proposalId]: bucket };
    if (Object.keys(newAssignments).length === totalProposals) {
...
const handleUnassign = (proposalId) => {
  ...
  // auto-expand related category
  const foundCategory = Object.entries(proposalsData).find(([cat, proposals]) =>
    proposals.some(p => p.id.toString() === proposalId)
  )?.[0];

  if (foundCategory) {
    // Defer expansion for first category to prevent scroll jump
    if (foundCategory === 'Economic Mobility and Growth') {
      setTimeout(() => {
        if (foundCategory === 'Economic Mobility and Growth') {
  setTimeout(() => {
    setExpandedCategories(prev => ({ ...prev, [foundCategory]: true }));
  }, 0);
} else {
  setExpandedCategories(prev => ({ ...prev, [foundCategory]: true }));
}
...
{totalAssigned === totalProposals && !readyToSubmit && !submitted && (
